...
class Iterator {
   ...
   public:
    ...
    const T* base() {
        return iterator_;
    }  // return a pointer that has adress iterator points to

    T& operator*() {
        return *iterator_;
    }  // overload the operator * which returns a value of pointer by reference(we can change the value that iterator points + we don't make the copy)

    T* operator->() {
        return iterator_;
    }  // return the pointer that contains adress
